...
import com.madjiejie.answer.modules.manuscript.ui.NoticeManuscriptActivity;
import com.madjiejie.answer.modules.more.MoreActivity;
import com.madjiejie.answer.modules.notice.ui.NewNoticeActivity;
import com.madjiejie.answer.modules.user.ui.TeacherInfoActivity;
import com.madjiejie.answer.utils.often.ImagesUtils;

import static com.madjiejie.answer.base.Constant.MODE;


public class TeacherDrawerAdapter extends BaseAdapter
{
	...
	private static final int NewQuestionActivity = 2;
	private static final int ManuscriptActivity = 3;
	private static final int MORE_ACTIVITY = 4;
	
	
	public TeacherDrawerAdapter ( Context cxt, DrawerLayout drawer )
	{
		this.cxt = cxt;
		this.drawer = drawer;
		initdates();
	...
	private void initdates ()
	{
		...
		strs[1] = cxt.getResources().getString(R.string.notice);
...
//		pics[2] = cxt.getResources().getDrawable(R.drawable.ic_enshrine);
	...
	private View type0 ( int i, View view, ViewGroup viewGroup )
	{
		...
		int sex = Constant.sTeacher.sex.equals("男") ? Constant.MAN : Constant.WOMAN;
		
		holder.iv_avatar.setImageBitmap(ImagesUtils.drawHeadIcon(cxt, Constant.sTeacher.name, false, sex));
		holder.tv_name.setText(Constant.sTeacher.name);
		
		view.setOnClickListener(v ->
		{
			cxt.startActivity(new Intent(cxt, TeacherInfoActivity.class));
	...
	private View type1 ( final int i, View view, ViewGroup viewGroup )
	{
		...
		view.setOnClickListener(v ->
		{
			...
			switch ( i )
			{
				case NewQuestionActivity:
					intent = new Intent(cxt, NewNoticeActivity.class);
					break;
				case ManuscriptActivity:
					intent = new Intent(cxt, NoticeManuscriptActivity.class);
					break;
              case MORE_ACTIVITY:
			...
			if ( intent != null )
			{
				intent.putExtra(MODE, Constant.USER_TYPE);
